...
"""
ui/voice.py — Modo Conversa: avatar animado + microfone contínuo (VAD).
Refatorado com st.fragment para isolar o processamento de áudio.

MUDANÇAS vs versão anterior:
  - @st.fragment no processamento de áudio → microfone reage sem recarregar
    o avatar HTML inteiro (que é pesado — ~900px de iframe)
  - O iframe do avatar só é recriado quando realmente muda algo (tts_b64, reply)
...
"""
ui/voice.py — Modo Conversa: avatar animado + microfone contínuo.
Mantém: fixFooter, resizeIframe, múltiplas chaves API, sem botão Close.
"""
...
# ── Altura do iframe (lida do query param ?vh=) ───────────────────────────────
_iframe_h = 900
try:
    _qp = st.query_params.get("vh")
    if _qp:
        _iframe_h = max(500, int(_qp) - 50)
except Exception:
    pass

# ── Chave API com suporte a múltiplas chaves ──────────────────────────────────

def _get_api_key() -> str:
    for name in ["ANTHROPIC_API_KEY"] + [f"ANTHROPIC_API_KEY_{i}" for i in range(2, 5)]:
        v = os.getenv(name, "")
        if v:
            return v
    try:
...
header[data-testid="stHeader"],#MainMenu,footer,header{
...
[data-testid="stToolbar"]{display:none!important;}
    ...
    if st.button("✕ Fechar", key="vm_close_btn"):
    ...
    speaking_        = json.dumps(t("speaking_ai",  lang))
    has_anim_js      = "false"
    av_normal_js     = json.dumps("")
    av_meio_js       = json.dumps("")
    av_aberta_js     = json.dumps("")
    av_bem_aberta_js = json.dumps("")
    av_ouvindo_js    = json.dumps("")
    av_piscando_js   = json.dumps("")
    av_surpresa_js   = json.dumps("")
    photo_js         = json.dumps(get_tati_mini_b64() or get_photo_b64())
    prof_name_js     = json.dumps(PROF_NAME)
    good_pronunc_js  = json.dumps(bool(st.session_state.get("_vm_good_pronunciation", False)))
    st.session_state.pop("_vm_good_pronunciation", None)
...
.audio-controls{{display:flex;align-items:center;gap:6px;padding:8px 12px;background:#0d1420;border:1px solid #1a2535;border-radius:12px;width:100%;overflow-x:auto;overflow-y:hidden;-webkit-overflow-scrolling:touch;white-space:nowrap;scrollbar-width:none;flex-wrap:nowrap;min-height:44px;}}
.audio-controls::-webkit-scrollbar{{display:none;}}
@media(max-width:400px){{.audio-controls{{padding:6px 10px;gap:4px;}}.ctrl-label{{font-size:.6rem;}}.ctrl-val{{font-size:.6rem;min-width:24px;}}#global-play-btn{{padding:4px 10px;font-size:.72rem;}}}}
.ctrl-label{{font-size:.68rem;color:#4a5a6a;white-space:nowrap;flex-shrink:0;}}.ctrl-val{{font-size:.68rem;color:#8b949e;min-width:28px;text-align:left;flex-shrink:0;}}
input[type=range].ctrl-range{{-webkit-appearance:none;flex-shrink:0;width:60px;height:4px;background:#1a2535;border-radius:2px;outline:none;cursor:pointer;touch-action:none;}}
@media(min-width:480px){{input[type=range].ctrl-range{{width:80px;}}}}
input[type=range].ctrl-range::-webkit-slider-thumb{{-webkit-appearance:none;width:16px;height:16px;border-radius:50%;background:{ring_color};cursor:pointer;}}
...
<script>
(function(){{
var TTS_B64={tts_js},REPLY={reply_js},HISTORY={history_js},VM_ERROR={err_js};
var TAP_SPEAK={tap_speak},TAP_STOP={tap_stop},SPEAKING={speaking_};
var HAS_ANIM={has_anim_js},GOOD_PRONUNC={good_pronunc_js},PHOTO={photo_js},PROF_NAME={prof_name_js};
var F_NORMAL={av_normal_js},F_MEIO={av_meio_js},F_ABERTA={av_aberta_js},F_BEM_ABERTA={av_bem_aberta_js};
var F_OUVINDO={av_ouvindo_js},F_PISCANDO={av_piscando_js},F_SURPRESA={av_surpresa_js};
...
function _stopAllTimers(){{if(_blinkTimer){{clearTimeout(_blinkTimer);clearInterval(_blinkTimer);_blinkTimer=null;}}if(_mouthTimer){{clearInterval(_mouthTimer);_mouthTimer=null;}}}}
function enterIdle(){{_stopAllTimers();_state='idle';setFrame(F_NORMAL);ring.classList.remove('active');statusTxt.textContent='● Online';if(!HAS_ANIM)return;(function scheduleBlink(){{_blinkTimer=setTimeout(function(){{if(_state!=='idle')return;setFrame(F_PISCANDO);setTimeout(function(){{if(_state!=='idle')return;setFrame(F_NORMAL);scheduleBlink();}},150);}},3210+Math.random()*2000);}})();}}
function enterListening(){{_stopAllTimers();_state='listening';setFrame(HAS_ANIM?F_OUVINDO:PHOTO);ring.classList.remove('active');statusTxt.textContent='🎙 Ouvindo…';}}
function enterProcessing(){{_stopAllTimers();_state='processing';setFrame(HAS_ANIM?F_NORMAL:PHOTO);ring.classList.remove('active');statusTxt.textContent='⏳ Processando…';if(!HAS_ANIM)return;_blinkTimer=setInterval(function(){{if(_state!=='processing')return;setFrame(F_PISCANDO);setTimeout(function(){{if(_state!=='processing')return;setFrame(F_NORMAL);}},180);}},2200);}}
function enterSpeaking(audioEl){{_stopAllTimers();_state='speaking';ring.classList.add('active');statusTxt.textContent=SPEAKING;if(!HAS_ANIM)return;try{{if(!_audioCtx)_audioCtx=new(window.AudioContext||window.webkitAudioContext)();if(!_analyser){{_analyser=_audioCtx.createAnalyser();_analyser.fftSize=1024;_analyser.smoothingTimeConstant=0.1;var src=_audioCtx.createMediaElementSource(audioEl);src.connect(_analyser);_analyser.connect(_audioCtx.destination);}}var buf=new Uint8Array(_analyser.frequencyBinCount);_mouthTimer=setInterval(function(){{if(_state!=='speaking')return;_analyser.getByteFrequencyData(buf);var sum=0,n=Math.min(100,buf.length);for(var i=4;i<n;i++)sum+=buf[i];setFrame((sum/(n-4))<18?F_NORMAL:F_MEIO);}},60);}}catch(e){{_mouthFallbackIdx=0;_mouthTimer=setInterval(function(){{if(_state!=='speaking')return;setFrame(_mouthFallbackIdx++%2===0?F_MEIO:F_NORMAL);}},250);}}}}
function onSpeakingEnded(goodPronunc){{_stopAllTimers();_analyser=null;if(goodPronunc&&F_BEM_ABERTA){{setFrame(F_BEM_ABERTA);setTimeout(enterIdle,1200);}}else enterIdle();}}

var currentAudio=null,lastB64=null;
function getVol(){{return parseFloat(document.getElementById('vol-slider').value)||1;}}
function getSpd(){{return parseFloat(document.getElementById('spd-slider').value)||1;}}
function updateGlobalBtn(playing){{var btn=document.getElementById('global-play-btn');if(!btn)return;btn.textContent=playing?'⏹ Parar':'▶ Ouvir';btn.style.background=playing?'#8b2a2a':'#1a2535';}}
function playTTS(b64,onEndCallback){{if(currentAudio){{currentAudio.pause();currentAudio=null;}}_analyser=null;if(!b64)return;lastB64=b64;var audio=new Audio('data:audio/mp3;base64,'+b64);audio.volume=getVol();audio.playbackRate=getSpd();audio._srcB64=b64;currentAudio=audio;audio.onplay=function(){{enterSpeaking(audio);updateGlobalBtn(true);}};audio.onended=function(){{currentAudio=null;updateGlobalBtn(false);onSpeakingEnded(GOOD_PRONUNC);if(onEndCallback)onEndCallback();}};audio.onerror=function(){{currentAudio=null;updateGlobalBtn(false);enterIdle();}};audio.play().catch(function(){{currentAudio=null;updateGlobalBtn(false);enterIdle();}});}}
function stopTTS(){{if(currentAudio){{currentAudio.pause();currentAudio=null;}}_analyser=null;updateGlobalBtn(false);enterIdle();}}
document.getElementById('global-play-btn').addEventListener('click',function(){{if(currentAudio&&!currentAudio.paused)stopTTS();else if(lastB64||TTS_B64)playTTS(lastB64||TTS_B64);}});
document.getElementById('vol-slider').addEventListener('input',function(){{document.getElementById('vol-val').textContent=Math.round(this.value*100)+'%';if(currentAudio)currentAudio.volume=parseFloat(this.value);}});
document.getElementById('spd-slider').addEventListener('input',function(){{document.getElementById('spd-val').textContent=parseFloat(this.value).toFixed(1)+'x';if(currentAudio)currentAudio.playbackRate=parseFloat(this.value);}});

function addBubble(role,text,b64){{var label=document.createElement('div');label.className='bubble-label'+(role==='user'?' right':'');label.textContent=role==='user'?'Você':PROF_NAME;var bub=document.createElement('div');bub.className='bubble '+role;bub.textContent=text;histWrap.appendChild(label);histWrap.appendChild(bub);if(role==='bot'&&b64){{var pbtn=document.createElement('button');pbtn.className='bubble-play-btn';pbtn.textContent='▶ Ouvir';pbtn.addEventListener('click',function(){{var isPlaying=currentAudio&&!currentAudio.paused&&currentAudio._srcB64===b64;if(isPlaying){{stopTTS();pbtn.textContent='▶ Ouvir';pbtn.classList.remove('playing');}}else{{document.querySelectorAll('.bubble-play-btn').forEach(function(b){{b.textContent='▶ Ouvir';b.classList.remove('playing');}});pbtn.textContent='⏹ Parar';pbtn.classList.add('playing');playTTS(b64,function(){{pbtn.textContent='▶ Ouvir';pbtn.classList.remove('playing');}});}}}});histWrap.appendChild(pbtn);}}setTimeout(function(){{histWrap.scrollTop=histWrap.scrollHeight;}},60);}}

if(VM_ERROR){{errBox.textContent=VM_ERROR;errBox.style.display='block';enterIdle();}}
else{{errBox.style.display='none';if(HISTORY&&HISTORY.length>0){{HISTORY.forEach(function(msg){{addBubble(msg.role==='user'?'user':'bot',msg.content,msg.tts_b64||'');}});}}if(TTS_B64)setTimeout(function(){{playTTS(TTS_B64);}},300);else enterIdle();}}

var recording=false;
function getRealMicBtn(){{var doc=window.parent.document;var ai=doc.querySelector('[data-testid="stAudioInput"]');if(!ai)return null;return ai.querySelector('button')||ai.querySelector('[data-testid="stAudioInputRecordButton"]');}}
micBtn.addEventListener('click',function(){{var realBtn=getRealMicBtn();if(!realBtn)return;if(recording){{recording=false;micBtn.classList.remove('recording');micBtn.classList.add('processing');micBtn.innerHTML='<i class="fa-solid fa-spinner fa-spin"></i>';micHint.textContent=TAP_SPEAK;enterProcessing();realBtn.click();}}else{{if(currentAudio){{currentAudio.pause();currentAudio=null;}}if(window.parent.speechSynthesis)window.parent.speechSynthesis.cancel();recording=true;micBtn.classList.remove('processing');micBtn.classList.add('recording');micBtn.innerHTML='<i class="fa-solid fa-stop"></i>';micHint.textContent=TAP_STOP;enterListening();realBtn.click();}}}});

function hideNativeAudio(){{var doc=window.parent.document;var ai=doc.querySelector('[data-testid="stAudioInput"]');if(ai){{ai.style.cssText='position:fixed;bottom:-999px;left:-9999px;opacity:0;pointer-events:none;width:1px;height:1px;';var btn=ai.querySelector('button');if(btn)btn.style.pointerEvents='auto';}}}}
hideNativeAudio();
try{{var obs=new MutationObserver(hideNativeAudio);obs.observe(window.parent.document.body,{{childList:true,subtree:true}});setTimeout(function(){{obs.disconnect();}},15000);}}catch(e){{}}

function fixFooter(){{
    var footer=document.querySelector('.mic-footer'),hw=document.getElementById('historyWrap');
    if(!footer||!hw)return;
    footer.style.cssText=['position:fixed','bottom:0','left:0','right:0','width:100%','max-width:100%','z-index:9999','background:#060a10','border-top:1px solid #1a2535','box-sizing:border-box','display:flex','flex-direction:column','align-items:center','gap:8px','padding:10px 16px 14px','height:auto','overflow:visible'].map(function(r){{return r+'!important';}}).join(';')+';';
    footer.querySelectorAll('*').forEach(function(el){{el.style.setProperty('display',el.tagName==='INPUT'?'block':el.classList.contains('audio-controls')?'flex':el.classList.contains('mic-btn')?'flex':'block','important');el.style.setProperty('visibility','visible','important');el.style.setProperty('opacity','1','important');el.style.setProperty('height','auto','important');el.style.setProperty('overflow','visible','important');}});
    var mb=footer.querySelector('.mic-btn');
    if(mb){{mb.style.setProperty('width','72px','important');mb.style.setProperty('height','72px','important');mb.style.setProperty('min-width','72px','important');mb.style.setProperty('min-height','72px','important');mb.style.setProperty('font-size','28px','important');mb.style.setProperty('border-radius','50%','important');mb.style.setProperty('flex-shrink','0','important');}}
    [100,350,700].forEach(function(d){{setTimeout(function(){{var fh=footer.getBoundingClientRect().height||140;hw.style.setProperty('padding-bottom',(fh+16)+'px','important');hw.style.setProperty('overflow-y','auto','important');hw.scrollTop=hw.scrollHeight;}},d);}});
}}

(function resizeIframe(){{
    try{{var vh=window.parent.innerHeight;if(window.parent.visualViewport)vh=Math.round(window.parent.visualViewport.height);var url=new URL(window.parent.location.href);if(url.searchParams.get('vh')!==String(vh)){{url.searchParams.set('vh',vh);window.parent.history.replaceState(null,'',url.toString());}}}}catch(e){{}}
    try{{var par=window.parent,h=par.innerHeight;try{{if(par.visualViewport)h=par.visualViewport.height;}}catch(e){{}}var iframes=par.document.querySelectorAll('iframe');for(var i=0;i<iframes.length;i++){{try{{if(iframes[i].contentWindow===window){{iframes[i].style.cssText=['height:'+h+'px','max-height:'+h+'px','min-height:200px','display:block','border:none','width:100%'].join(';');var p=iframes[i].parentElement;for(var j=0;j<10&&p&&p!==par.document.body;j++){{p.style.margin=p.style.padding='0';p.style.overflow='hidden';p.style.maxHeight=h+'px';p=p.parentElement;}}break;}}}}catch(e){{}}}}}}catch(e){{}}
    fixFooter();
    try{{var par2=window.parent;par2.removeEventListener('resize',resizeIframe);par2.addEventListener('resize',resizeIframe);if(par2.visualViewport){{par2.visualViewport.removeEventListener('resize',resizeIframe);par2.visualViewport.addEventListener('resize',resizeIframe);}}}}catch(e){{}}
}})();

try{{var _ro=new ResizeObserver(function(){{fixFooter();}});_ro.observe(document.body);}}catch(e){{window.addEventListener('resize',fixFooter);}}

}})();
</script></body></html>""", height=_iframe_h, scrolling=False)